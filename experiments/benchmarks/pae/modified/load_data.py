...
def load_celeba(data_dir, flatten=False, params=None):
    ...
    try:
        x_test  = np.load(os.path.join(data_dir, 'celeba%d'%dimensions,'train.npy'))
    except:
        print('loading test data')
        x_test  = load_celeba_data(data_dir,'test', dimensions)
        np.save(os.path.join(data_dir, 'celeba%d'%dimensions, 'train.npy'), x_test)
    try:
...
def load_gan64(data_dir, flatten=False, params=None):
    try:
        x_val = 255*np.load(os.path.join(data_dir, 'gan64', 'val.npy'))
        x_val = x_val.transpose(0,2,3,1)
    except:
        raise ValueError(f"Didn't find validation set for gan64 dataset")
    try:
        x_test = 255*np.load(os.path.join(data_dir, 'gan64', 'train.npy'))
        x_test = x_test.transpose(0,2,3,1)
    except:
        raise ValueError(f"Didn't find test set for gan64 dataset")
    try:
        x_train = 255*np.load(os.path.join(data_dir, 'gan64', 'train.npy'))
        x_train = x_train.transpose(0,2,3,1)
    except:
        raise ValueError(f"Didn't find train set for gan64 dataset")

    return  x_train, None, x_test, None, x_val, None

def load_gan2(data_dir, flatten=False, params=None):
    try:
        x_val   = 255*np.load(os.path.join(data_dir, 'gan2', 'val.npy'))
        x_val = x_val.transpose(0,2,3,1)
    except:
        raise ValueError(f"Didn't find validation set for gan2 dataset")
    try:
        x_test  = 255*np.load(os.path.join(data_dir, 'gan2', 'train.npy'))
        x_test = x_test.transpose(0,2,3,1)
    except:
        raise ValueError(f"Didn't find test set for gan2 dataset")
    try:
        x_train = 255*np.load(os.path.join(data_dir, 'gan2', 'train.npy'))
        x_train = x_train.transpose(0,2,3,1)
    except:
        raise ValueError(f"Didn't find train set for gan2 dataset")
