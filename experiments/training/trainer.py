...
class Trainer(BaseTrainer):
    ...
    def train(
        ...
        torch.manual_seed(seed)
        """
        logger.info("Initialising training data")
        ...
        logger.info("Setting up LR scheduler")
        if epochs < 2:
            scheduler = None
        ...
        if early_stopping and early_stopping_patience is None:
            logger.info("Using early stopping with infinite patience")
        elif early_stopping:
            logger.info("Using early stopping with patience %s", early_stopping_patience)
        else:
        ...
        logger.info("Beginning main training loop")
        ...
        for i_epoch in range(initial_epoch, epochs):
            logger.info("Training epoch %s / %s", i_epoch + 1, epochs)

            # LR schedule
            if sched is not None:
                logger.info("Learning rate: %s", sched.get_last_lr())
