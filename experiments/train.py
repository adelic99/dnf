...
from training import ForwardTrainer, ConditionalForwardTrainer, SCANDALForwardTrainer, AdversarialTrainer, ConditionalAdversarialTrainer, AlternatingTrainer, SamplingForwardTrainer
...
def parse_args():
    ...
    parser.add_argument(
        "--algorithm", type=str, default="dnf", choices=ALGORITHMS, help="Model: flow (AF), mf (FOM, M-flow), emf (Me-flow), pie (PIE), gamf (M-flow-OT), pae (PAE)...",
    )
    ...
    # Dataset details
    parser.add_argument("--truelatentdim", type=int, default=1, help="True manifold dimensionality (for datasets where that is variable)")
    ...
    # Model details
    parser.add_argument("--modellatentdim", type=int, default=1, help="Model manifold dimensionality")
    ...
    parser.add_argument("--lineartransform", type=str, default="lu", help="Scalar linear trf: linear | permutation")
    parser.add_argument("--outerlayers", type=int, default=9, help="Number of transformations in f (not counting linear transformations)")
    parser.add_argument("--innerlayers", type=int, default=1, help="Number of transformations in h (not counting linear transformations)")
    ...
    parser.add_argument("--splinebins", default=5, type=int, help="Number of spline bins")
    ...
    # DNF additions
    parser.add_argument("--sig2", type=float, default=0.01, help="noise magnitude sigma^2")
    ...
    parser.add_argument("--noise_type", type=str, default="gaussian", help="Noise type: gaussian, true_normal (only for thin_spiral)")
    parser.add_argument("--sampling_train", action="store_true", help="Sample data for every epoch")
...
def train_manifold_flow_alternating(args, dataset, model, simulator):
    ...
    meta_kwargs = {"dataset": dataset, "initial_lr": args.lr, "scheduler": optim.lr_scheduler.CosineAnnealingLR, "validation_split": args.validationsplit, "sig2": args.sig2}
    if args.weightdecay is not None:
...
def train_dnf(args, dataset, model, simulator):
    ...
    learning_curves = trainer.train(
        ...
        parameters=list(model.parameters()),
        callbacks=callbacks_,
        forward_kwargs={"mode": str(args.algorithm), "return_hidden": args.uvl2reg is not None},
        initial_epoch=args.startepoch,
...
def train_dnf_sampling(args, dataset, model, simulator):
    """ DNF training with sampling rather than fixed dataset"""
    trainer = SamplingForwardTrainer(model) if simulator.parameter_dim() is None else ConditionalForwardTrainer(model) if args.scandal is None else SCANDALForwardTrainer(model)
    common_kwargs, scandal_loss, scandal_label, scandal_weight = make_training_kwargs(args, dataset)
    callbacks_ = [callbacks.save_model_after_every_epoch(create_filename("checkpoint", None, args))]
    if simulator.is_image():
        callbacks_.append(
...
def train_dnf_dec(args, dataset, model, simulator):
    """ DNF with decoder training """
    trainer = ForwardTrainer(model) if simulator.parameter_dim() is None else ConditionalForwardTrainer(model) if args.scandal is None else SCANDALForwardTrainer(model)
    common_kwargs, scandal_loss, scandal_label, scandal_weight = make_training_kwargs(args, dataset)
    callbacks_ = [callbacks.save_model_after_every_epoch(create_filename("checkpoint", None, args))]
    if simulator.is_image():
    ...
    logger.info("Starting training denoising flow with seperate decoder on NLL")
    learning_curves = trainer.train(
        loss_functions=[losses.nll] + [losses.mse] + ([] if args.uvl2reg is None else [losses.hiddenl2reg]),
        loss_labels=["NLL"] + ["MSE"] + ([] if args.uvl2reg is None else ["L2_lat"]),
        loss_weights=[1] + [args.msefactor] + ([] if args.uvl2reg is None else [args.uvl2reg]),
        ...
        parameters=list(model.parameters()),
        callbacks=callbacks_,
        forward_kwargs={"mode": "dnf_dec", "return_hidden": args.uvl2reg is not None},
        initial_epoch=args.startepoch,
        restart_scheduler = args.scheduler_restart,
        **common_kwargs,
    )
...
def train_model(args, dataset, model, simulator):
    ...
    elif args.algorithm in ["dnf","dnf_v"]:
        if args.sampling_train:
            learning_curves = train_dnf_sampling(args, dataset, model, simulator)
        else: 
            learning_curves = train_dnf(args, dataset, model, simulator)
    elif args.algorithm == "dnf_dec":
        learning_curves = train_dnf_dec(args, dataset, model, simulator)
    else:
