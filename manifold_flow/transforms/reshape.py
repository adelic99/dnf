...
class ReshapeTransform(transforms.Transform):
    ...
    def forward(self, inputs, context=None):
        if tuple(inputs.shape[1:]) != self.input_shape:
            raise RuntimeError("Unexpected inputs shape ({}, but expecting {})".format(tuple(inputs.shape[1:]), self.input_shape))
        return inputs.reshape(-1, *self.output_shape), torch.zeros(inputs.shape[0])

    def inverse(self, inputs, context=None):
        if tuple(inputs.shape[1:]) != self.output_shape:
            raise RuntimeError("Unexpected inputs shape ({}, but expecting {})".format(tuple(inputs.shape[1:]), self.output_shape))
